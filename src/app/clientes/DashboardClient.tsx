...
// calculateBalances pode receber 2 ou 3 args dependendo da versão — chamamos com spread
...
export default function DashboardClient({ orders, payments, clients }: Props) {
  ...
  // Support both 2-arg and 3-arg versions of calculateBalances
  const balances = (billing as any).calculateBalances(filteredOrders, filteredPayments, 0);
  const cycles = balances.cycles ?? [];
  const totalOpen = balances.totalOpen ?? 0;
  const nextDue = balances.nextDue ?? 0;
  const overdue = balances.overdue ?? 0;
          ...
          <tbody>
            ...
            {cycles.map((cycle: any) => (
              <tr key={cycle.due_date} className="border-t hover:bg-gray-50">
                ...
                <td className="px-4 py-3 text-right font-semibold">{formatCurrency(cycle.balance)}</td>
